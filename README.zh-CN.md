...
- 在 `/var/tmp`（或 `/tmp`）与 `/usr/local` 所在文件系统上预留约 **90 MiB** 可用空间：固定版本压缩包、从中解出的可执行文件，以及最终发布的副本会同时存在。安装器会在下载前检查文件系统容量，但该检查只是预估，因为项目或卷配额可能不会反映在空闲空间计数中。如果实际提取写入中途停止，安装器会把它报告为不完整的存储写入并列出已写入／预期字节数，而不会误报为发布资产损坏。
...
| 提示空间不足，或文件未能写完 | 释放空间，或提高容器／项目／卷磁盘配额。预检查会在可用时报告文件系统容量；即使预检查显示空间充足，实际短写仍会报告已写入和预期字节数，因为部分配额对 `statfs` 不可见。 |
