...
ROOT=${1:?usage: post_install_audit.sh ROOT PASSFILE}
PASSFILE=${2:?usage: post_install_audit.sh ROOT PASSFILE}
...
if pass_leaks "$ROOT/var/log" "$ROOT/etc/systemd"; then
...
grep -qF 'ExecStart=/usr/local/libexec/xray-socks5/xray run -c /etc/xray-socks5/config.json' "$unit"
