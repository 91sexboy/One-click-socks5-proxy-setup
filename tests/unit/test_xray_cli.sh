...
# s5t_cli <stdin-text> <argv...> : run socks5.sh as a child, leaving T_OUT/T_STATUS.
...
s5t_cli '2
' status
assert_eq "status asks for language exactly once" 1 "$(s5t_prompt_count "$T_OUT")"
assert_not_contains "one answer is enough for status" 'invalid language' "$T_OUT"

s5t_cli '2
' help
assert_eq "help asks for language exactly once" 1 "$(s5t_prompt_count "$T_OUT")"
...
2
' status
assert_eq "an invalid language answer is retried" 2 "$(s5t_prompt_count "$T_OUT")"
...
assert_eq "language retries are bounded at three attempts" 3 "$(s5t_prompt_count "$T_OUT")"

s5t_cli '' status
assert_ne "EOF on the language prompt fails" 0 "$T_STATUS"
assert_eq "EOF is not retried" 1 "$(s5t_prompt_count "$T_OUT")"
...
assert_eq "a later invocation does not ask for language" 0 "$(s5t_prompt_count "$T_OUT")"
...
assert_eq "unprivileged help does not prompt again" 0 "$(s5t_prompt_count "$T_OUT")"
s5t_cli '2
' language
...
assert_eq "a malformed preference prompts exactly once" 1 "$(s5t_prompt_count "$T_OUT")"
...
s5t_cli '1
' help
assert_eq "a writable preference is not trusted" 1 "$(s5t_prompt_count "$T_OUT")"
