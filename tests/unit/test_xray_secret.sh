...
s5t_secret_absent "a var/log leak is reported without the credential" "$T_OUT"

s5t_build_tree
printf 'Environment=PASS=%s\n' "$SECRET" >>"$UNIT"
s5t_audit
assert_ne "a credential in the unit fails the audit" 0 "$T_STATUS"
s5t_secret_absent "a unit leak is reported without the credential" "$T_OUT"
...
_chmod_at=$(s5t_line_of "$_engine" 'chmod 0600 "$WORK/config.json"')
_write_at=$(s5t_line_of "$_engine" 'cat >"$WORK/config.json"')
...
s5t_secret_absent "run-socks5.sh redacts the status and journal dumps on failure" "$_runout"
...
s5t_runner_no_secret "failure stdout is redacted" "$_runstdout"
s5t_runner_no_secret "failure stderr and service diagnostics are redacted" "$_runstderr"
case "$_runstderr" in
    *'normal command diagnostic'*'--- systemctl status ---'*'--- journal ---'*'--- listener ---'*) t_ok ;;
    *) t_bad "failure retains command and service diagnostics" ;;
esac
s5t_runner_capture --diagnose
...
s5t_runner_no_secret "shell-level errors are redacted" "$(cat "$_rundir/error-output")"
...
s5t_runner_both_hidden "rotation success stdout hides both generations" "$_runstdout"
s5t_runner_both_hidden "rotation success stderr hides both generations" "$_runstderr"
case "$_runstdout" in
...
s5t_runner_both_hidden "rotation failure stdout hides both generations" "$_runstdout"
s5t_runner_both_hidden "rotation failure diagnostics hide both generations" "$_runstderr"
case "$_runstderr" in
...
s5t_runner_generations --diagnose
assert_eq "rotation standalone diagnostics fail" 1 "$_runstatus"
s5t_runner_both_hidden "rotation standalone diagnostics hide both generations" "$_runstderr"
s5t_runner_both_hidden "both generations stay out of redactor argv" "$(cat "$_rundir/argv")"
printf 'incomplete\n' >"$_rundir/pass.next"
s5t_runner_generations
assert_eq "invalid extra credential file fails before replay" 2 "$_runstatus"
s5t_runner_no_secret "invalid extra credential file exposes no old credential" "$_runstderr"
